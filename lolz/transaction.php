...
<p style="float: right; padding-right: 8px;margin-top: 15px; margin-bottom: 0px;  font-size: 20px;">Welcome admin:<strong><?php echo $_SESSION["username"]; ?></strong> of Company:<strong><?php echo $_SESSION["cname"]; ?></strong></p>
  <?php if(isset($_SESSION['success'])){
  	...
  	$sql="select * 
  		...
  		WHERE T.c_name IN ( SELECT c_name
  							FROM companies AS C
  							WHERE C.c_name IN ( SELECT c_name
  												FROM admin AS A
  												WHERE A.loginID = '$identity'));";
    $q=mysqli_query($conn,$sql);
  	$x=$q->fetch_assoc();
  	?>
<div class="container">
<h2>Transation Details of <?php echo $_SESSION["cname"]; ?></h2>
<div>
	<h4><pre>Sale Quantity: 				<?php echo $x["sale_quantity"]; ?></pre></h4>
	<hr>
	<h4><pre>Total Sale: 				      <?php echo $x["total_sale"]; ?></pre></h4>
	<hr>
	<h4><pre>Holdings Cost: 				<?php echo $x["holdings_cost"]; ?></pre></h4>
	<hr>
	<h4><pre>Holdings quantitiy: 				<?php echo $x["holdings_qty"]; ?></pre></h4>
	<hr>
	<h4><pre>Gain/Loss: 				<?php echo $x["gain_loss"]; ?></pre></h4>
	<hr>
	<hr>
</div>
