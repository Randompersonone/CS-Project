...
<p style="float: right; padding-right: 8px;margin-top: 15px; margin-bottom: 0px;  font-size: 20px;">Welcome <strong><?php echo $_SESSION["username"]; ?></strong></p>
  <?php if(isset($_SESSION['success'])){
  	...
  	$sql="select * FROM user_balance WHERE loginID = '$identity';";
    $q=mysqli_query($conn,$sql);
  	$x=$q->fetch_assoc();
  	?>
<div class="container">
<h2>Balance account of <?php echo $_SESSION["username"]; ?></h2>
<div>
	<h4><pre>User ID: 				<?php echo $x["loginID"]; ?></pre></h4>
	<hr>
	<h4><pre>Name: 				      <?php echo $x["serial_no"]; ?></pre></h4>
	<hr>
	<h4><pre>User Shares: 				<?php echo $x["U_shares"]; ?></pre></h4>
	<hr>
	<h4><pre>Money Spent: 				<?php echo $x["money_spent"]; ?></pre></h4>
	<hr>
	<h4><pre>Balance: 				<?php echo $x["balance"]; ?></pre></h4>
	<hr>
	<hr>
</div>
