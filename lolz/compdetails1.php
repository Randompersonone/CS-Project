<?php
include('compdetails2.php');
?>
<!DOCTYPE html>
<html>
<head>Company Details</title>
...
<p style="float: right; padding-right: 8px;margin-top: 15px; margin-bottom: 0px;  font-size: 20px;">Welcome <strong><?php echo $_SESSION["username"]; ?></strong></p>
  <?php if(isset($_SESSION['success'])){
    $comp=$_SESSION['comp'];
    $sql="SELECT open_,change_,price,p_change 
      FROM companies WHERE c_name='$comp';";
    $q=mysqli_query($conn,$sql);
    $x=$q->fetch_assoc();
    ?>
<div class="container">
<h2>Stock Details of <?php echo $comp ?></h2>
<div>
  <h4><pre>Opening Price:         <?php echo $x["open_"]; ?></pre></h4>
  <hr>
  <h4><pre>Change in price:              <?php echo $x["change_"]; ?></pre></h4>
  <hr>
  <h4><pre>Current Price:        <?php echo $x["price"]; ?></pre></h4>
  <hr>
  <h4><pre>Percentage change:         <?php echo $x["p_change"]; ?></pre></h4>
  <hr>
  <hr>
</div>
<p style="float: right; padding-right: 8px;margin-top: 15px; margin-bottom: 0px;  font-size: 20px;"><a href="compdetails.php"><strong>Go back</a></strong></p>
<?php } ?>
</body>
